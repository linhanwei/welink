...
public class GetPayList {
    ...
    @RequestMapping(value = {"/api/h/1.0/getPayList.htm"}, produces = "text/html;charset=utf-8")
    public String execute(HttpServletRequest request, HttpServletResponse response, ModelMap model) throws Exception {
        ...
        if (UserUtils.redirect(session)) {
            response.sendRedirect("/api/h/1.0/vLoginPage.htm?refurl=getPayList.htm");
            return null;
        }
        return "getPayList";
    }
