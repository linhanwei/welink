...
public class MyPoint {
    ...
    @RequestMapping(value = {"/api/h/1.0/myPoint.htm"}, produces = "text/html;charset=utf-8")
    public String execute(HttpServletRequest request, HttpServletResponse response, ModelMap model) throws Exception {
        ...
        if (UserUtils.redirect(session)) {
        	response.sendRedirect("/api/h/1.0/vLoginPage.htm?refurl=" + 
    				URLEncoder.encode("myPoint.htm"+(null==request.getQueryString() ? "" : "?"+request.getQueryString()), "utf-8"));
            return null;
        }
        return "myPoint";
    }
    
    @RequestMapping(value = {"/api/h/1.0/myPointList.htm"}, produces = "text/html;charset=utf-8")
    public String myPointList(HttpServletRequest request, HttpServletResponse response, ModelMap model) throws Exception {
        model.addAttribute("ver", new Date().getTime() / (1000 * 60 * 60 * 24));
        if (env.isProd()) {
            model.addAttribute(BizConstants.HOST, BizConstants.H5_SEVEN_NB_HOST);
        } else {
        ...
        if (UserUtils.redirect(session)) {
            response.sendRedirect("/api/h/1.0/vLoginPage.htm?refurl=myPointList.htm");
            return null;
        }
        return "myPoint";
    }
