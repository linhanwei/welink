...
public class SalesRecordList {
    ...
    @RequestMapping(value = {"/api/h/1.0/salesRecordList.htm"}, produces = "text/html;charset=utf-8")
    public String execute(HttpServletRequest request, HttpServletResponse response, ModelMap model) throws Exception {
        ...
        if (UserUtils.redirect(session)) {
            response.sendRedirect("/api/h/1.0/vLoginPage.htm?refurl=salesRecordList.htm");
            return null;
        }
        return "salesRecordList";
    }
    
    @RequestMapping(value = {"/api/h/1.0/returnSaleRecord.htm"}, produces = "text/html;charset=utf-8")
    public String returnSaleRecord(HttpServletRequest request, HttpServletResponse response, ModelMap model) throws Exception {
        model.addAttribute("ver", new Date().getTime() / (1000 * 60 * 60 * 24));
        if (env.isProd()) {
            model.addAttribute(BizConstants.HOST, BizConstants.H5_SEVEN_NB_HOST);
        } else {
        ...
        if (UserUtils.redirect(session)) {
            response.sendRedirect("/api/h/1.0/vLoginPage.htm?refurl=returnSaleRecord.htm");
            return null;
        }
