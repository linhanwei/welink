...
@Controller
public class DistributionExpertPage {
	@Resource
    private Env env;
	 ...
	 * @throws Exception
	 */
    @RequestMapping(value = {"/api/h/1.0/distributionExpertPage.htm"}, produces = "text/html;charset=utf-8")
    public String distributionExpertPage(HttpServletRequest request, HttpServletResponse response, ModelMap model) throws Exception {
        ...
        if (UserUtils.redirect(session)) {
            response.sendRedirect("/api/h/1.0/vLoginPage.htm?refurl=distributionExpertPage.htm");
            return null;
        }
        return "distributionExpertPage";
    }
}
