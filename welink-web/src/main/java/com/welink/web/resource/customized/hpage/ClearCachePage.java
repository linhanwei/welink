...
public class ClearCachePage {
	 ...
	 * @throws Exception
	 */
    @RequestMapping(value = {"/api/h/1.0/clearCachePage.htm"}, produces = "text/html;charset=utf-8")
    public String clearCachePage(HttpServletRequest request, HttpServletResponse response, ModelMap model) throws Exception {
        ...
        if (UserUtils.redirect(session)) {
            response.sendRedirect("/api/h/1.0/vLoginPage.htm?refurl=clearCachePage.htm");
            return null;
        }
