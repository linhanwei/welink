package com.welink.web.resource.customized.hpage;
...
@Controller
public class QuestionnaireReportPage {
	@Resource
    private Env env;
	 ...
	 * @throws Exception
	 */
    @RequestMapping(value = {"/api/h/1.0/questionnaireSurveyReportPage.htm"}, produces = "text/html;charset=utf-8")
    public String questionnaireSurveyReportPage(HttpServletRequest request, HttpServletResponse response, ModelMap model) throws Exception {
        ...
        if (UserUtils.redirect(session)) {
            response.sendRedirect("/api/h/1.0/vLoginPage.htm?refurl=questionnaireSurveyReportPage.htm");
            return null;
        }
        return "questionnaireSurveyReportPage";
    }
     ...
     * @throws Exception
     */
    @RequestMapping(value = {"/api/h/1.0/userReportPage.htm"}, produces = "text/html;charset=utf-8")
    public String userReportPage(HttpServletRequest request, HttpServletResponse response, ModelMap model) throws Exception {
        model.addAttribute("ver", new Date().getTime() / (1000 * 60 * 60 * 24));
        if (env.isProd()) {
            model.addAttribute(BizConstants.HOST, BizConstants.H5_SEVEN_NB_HOST);
        } else {
        ...
        if (UserUtils.redirect(session)) {
            response.sendRedirect("/api/h/1.0/vLoginPage.htm?refurl=userReportPage.htm");
            return null;
        }
